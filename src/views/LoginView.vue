  ...
  <div class="login-container">
    ...
    <div class="login-card">
      <div class="login-header">
        ...
        <h1 class="title">{{ siteSettings.name }}</h1>
        <div class="subtitle" v-html="siteSettings.subtitle"></div>
      </div>
      <el-form :model="loginForm" :rules="rules" ref="loginFormRef" label-position="top" class="login-form" @keyup.enter="handleLogin">
      ...
      <div class="login-footer">
        <div class="login-tip" v-html="renderMarkdown(siteSettings.loginTipMd)"></div>
...
const fetchSiteSettings = async () => {
  try {
    ...
    siteSettings.loginTipMd = response.data.login_tip_md || ''
  } catch (error) {
...
const handleLogin = async () => {
  ...
  await loginFormRef.value.validate(async (valid) => {
    if (valid) {
      loading.value = true
      try {
        const { success, error, user } = await authStore.login(loginForm.username, loginForm.password)
...
.login-tip a { color: #60a5fa; text-decoration: underline; }
.login-tip code { background: rgba(148,163,184,0.2); padding: 1px 4px; border-radius: 4px; }
.login-tip p { margin: 0.2rem 0; }
